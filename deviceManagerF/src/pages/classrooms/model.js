...
export default modelExtend(pageModel, {
    ...
    effects: {
      ...
      *delete({ payload }, { call, put, select }) {
        const data = yield call(removeClassrooms, objectToFormData({classroom_no: payload.classroom_no}))
        const { selectedRowKeys } = yield select(_ => _.classrooms)
        if (data.success) {
          yield put({
            type: 'updateState',
            payload: {
              selectedRowKeys: selectedRowKeys.filter(_ => _ !== payload.classroom_no),
            },
          })
        } else {
          throw data
